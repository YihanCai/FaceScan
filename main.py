...
import time
import cv2
from detector import HaarDetector, MediaPipeDetector
...
def get_detector(name: str):
    if name == "haar":
        ...
        raise ValueError(f"不支持的检测器: {name}，可选: haar, mediapipe")
...
def process_image(detector, source: str, output: str, save: bool, display: bool):
    ...
    for i, face in enumerate(result.faces):
        ...
        print(f"  人脸 {i + 1}: ({x}, {y}, {w}, {h}) 置信度: {conf:.4f}")
    if save or display:
        vis = draw_faces(result.image, result)
...
def main():
    ...
    parser.add_argument("--source", "-s", default="",
                        help="输入源：图片路径 / 视频路径 / 摄像头ID")
    parser.add_argument("--detector", "-d", choices=["haar", "mediapipe"],
    ...
    try:
        if args.mode == "image":
            ...
            if not os.path.isfile(src):
                print(f"文件不存在: {src}")
                sys.exit(1)
            process_image(detector, src, args.output, args.save, args.display)
